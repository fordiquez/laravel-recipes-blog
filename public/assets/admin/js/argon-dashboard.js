...
(function () {
    ...
    if (isWindows) {
        ...
        if (document.getElementsByClassName('navbar-collapse')[0]) {
            fixedPlugin = document.querySelector('.navbar:not(.navbar-expand-lg) .navbar-collapse');
            const ps = new PerfectScrollbar(fixedPlugin);
        }
...
// adding on inputs attributes for calling the focused and unfocused functions
...
function sidebarType(a) {
    ...
    if (color === 'bg-white') {
        ...
        const textDarks = document.querySelectorAll('.sidenav .text-dark');
    ...
    if (color === 'bg-default' && bodyDark) {
        const textDarks = document.querySelectorAll('.navbar-brand .text-dark');
        for (let i = 0; i < textDarks.length; i++) {
            textDarks[i].classList.add('text-white');
            textDarks[i].classList.remove('text-dark');
        }
